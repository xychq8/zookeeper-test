...
public class DistributedLock implements Watcher{
            ...
            new Thread(){
                ...
                public void run() {
                    try{
                        ...
                        dc.createConnection("127.0.0.1:2181", 10000);
    ...
    public boolean createPath(String path, String data, boolean needWatch) throws KeeperException, InterruptedException {
        if(zk.exists(path, needWatch) == null){
            String pathStr = this.zk.create(path, data.getBytes(), ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
            logger.info(LOG_PREFIX_OF_THREAD + "节点创建成功, path: " + pathStr + ", content: " + data);
        }
        return true;
    }
